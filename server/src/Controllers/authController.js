...
const signup = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const user = new Users({ contactName, phoneNumber, email: email.toLowerCase(), password: hashedPassword });
        await user.save();
...
const login = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        req.session.regenerate((err) => {
            ...
            const userData = { ...user.toObject(), password: undefined, __v: undefined, loginToken: undefined };
...
const updateAccountInfo = asyncHandler(async (req, res) => {
    const { contactName, phoneNumber, email, address1, address2, city, state, zipCode } = req.body;

    try {
        const user = await Users.findOne({ _id: req.session.user._id }).select('-password');
        ...
        req.session.user = user.toObject({ versionKey: false });

        return res.status(201).send({ message: "Fields have been updated", status: "success" });
    } catch (err) {
        console.log(err);
        res.status(403).send({ message: "Something went wrong while trying to update your account", status: "error" });
    }
});

const updateShippingInfo = asyncHandler(async (req, res) => {
    const { businessName, address1, address2, city, state, zipCode } = req.body;

    try {
        const user = await Users.findOne({ _id: req.session.user._id }).select('-password');

        user.businessName = businessName;
        user.shippingAddress1 = address1;
        user.shippingAddress2 = address2;
        user.shippingCity = city;
        user.shippingState = state;
        user.shippingZipCode = zipCode;
        ...
        console.log(err);
        res.status(403).send({ message: "Business name has already been taken", status: "error" });
    }
});
